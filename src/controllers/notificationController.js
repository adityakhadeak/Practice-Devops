...
const saveToken = async (req, res) => {
    try {
        ...
        const query = `INSERT INTO fcm_registration_tokens (user_id, token) VALUES( $1, $2) ON CONFLICT (token) DO UPDATE SET updated_at = CURRENT_TIMESTAMP RETURNING *`
...
const sendNotificationToMultipleDevice = async (req, res) => {
    try {
        ...
        const result = await pool.query('SELECT * FROM fcm_registration_tokens limit 500')
...
const subscribeToTopic = async (req, res) => {
    try {
        ...
        const result = await pool.query('SELECT * FROM fcm_registration_tokens')
